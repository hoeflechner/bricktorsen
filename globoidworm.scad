...
use <shortcuts.scad>  // help_shortcuts() and https://www.thingiverse.com/thing:644830
use <Naca_sweep.scad> // help_Naca_sweep() and https://www.thingiverse.com/thing:900137
// help_Naca_sweep();
// help_shortcuts();
...
// ratio
i = 5;          // gear ratio (4 <= i <= 100) slow render with high values
k = 1;           // # threads   (keep low) 
z = i*k;         // # teeth of wormgear (don't alter) !! CPU time grows with z !!
N = 50;          // refinement of donut for cutting worm and gear (default = 20)

// gear shape
m = 1;           // modulus (default = 2) better use scale() for resize
w = 0;          // attack angle (default = 20) 

// worm
min_r = 2;      // pitch circle radius of worm (default = 11)
max_r = 7;      // outer circle radius of worm (default = 15)
shaft_w = 2.4;   // shaft radius of worm
shaft_wn = 6;    // #edges of shaft bore
L = 2.1*m*z;     // length of worm, adjust factor for low and high ratio
cr = z+min_r;    // outer cycle radius for carving (don't alter)

// wormgear
h = 3;          // height of wormgear (default = 11) 
eng_t = 2;       // adjust to # of properly engaged theeth (proper render)
shaft_g = 0.5;   // shaft radius of worm gear
shaft_gn = 6;    // #edges of shaft bore 
clearance = .3;  // clearance (mm) of wormgear default (0.2)
